...
.eqv	display_buffer  0xFFFF000C  # endereco do buffer do display
	...
	msgE_data_hora_mal_formatada:            .asciiz "O formato da data ou hora est� incorreto."         
...
remover_livro:
	...
	la $t1, msgC_livro_removido     # Carrega o endere�co de msgC_livro_removido
	j escrever_com_sucesso_display
...
gerar_data_atual:
	...
	verificar_dias_restantes_ano:
   	...
   	calcular_mes:
   		...
   		addi $t0, $t0, 1  	# incrementando $t0 para corrigir a diferenca de 1 dia menos que tava sendo gerada antes dessa linha de c�digo existir
...
imprimir_data_hora_usuario:
	li $v0, 30   # Chama o syscall 30 para obter o hor�rio
    syscall
	...
	# A linha abaixo verifica se o tempo em $t0 � maior que 60 minutos 
	...
	ajustar_tempo:
		...
		lb $t3, hora_config_usuario      # Carrega o endereco de hora_config_usuario
		...
		bge $t0, 24, ajustar_hora   # Verifica se as horas decorridas � maior que 24 horas
		...
		ajustar_hora:
			...
			# N�o irei me dar ao trabalho de verificar se o dia, mes e ano s�o v�lidos, ap�s essa soma de dias , 
			# somei e j� t� bom demais, seria muito custoso fazer tudo isso j� que os dados da data e hora 
			# configurados pelo usuario n�o s�o salvos em arquivos .txt.
...
ajustar_data:
	...
	# O trecho abaixo compara se o usu�rio escreveu o argumento "--data"
	addi $s0, $s0, 1 	  # Passa um caractere para frente, por conta do espa�o entre os comandos
	la $s1, arg_data      # Carrega o endereco do argumento para comparar
	li $s2, 6             # Define a quantidade de caracteres em comando que ser� comparada
	jal comparar_strings
	beqz $v0, escrever_falta_argumento_data_display    # caso tenha faltado o argumento ou digitado incorretamente
	 
	addi $s0, $s0, 1 	# Passa um caractere para frente, por conta do espa�o entre os comando
	
	# O trecho abaixo verifica se os dados est�o no formato dd/mm/aaaa
	...
	# Vaso os caractere em $s0 n�o seja igual ao de $t1, imprime a mensagem de formato incorreto
	...
	la $t1, data_config_usuario   # Carrega o endereco de data_config_usuario  
	jal guardar_info_buffer  	  # Pula para a funcao que pega o que est� entre aspas e salva no data_config_usuario
	
	# O trecho abaixo compara se o usu�rio escreveu o argumento "--hora"
	addi $s0, $s0, 1 	  # Passa um caractere para frente, por conta do espa�o entre os comandos
	...
	# O trecho abaixo verifica se os dados est�o no formato hh:mm
	...
	la $t1, msgC_data_hora_configurada   # Carrega o endere�o de msgC_data_hora_configurada
	j escrever_com_sucesso_display
